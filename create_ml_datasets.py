...
def read_branch_trace(trace_path):
    ...
    return x['br_pc'].copy(), x['dir'].copy(), x['regs'].copy()


def create_new_dataset(dataset_path, pcs, directions ,regs):
    '''
    ...
    #print(pcs.shape[0])
    #pcs_column = np.full((regs.shape[0],1),pcs,dtype=np.uint32)
    processed_regs = np.hstack((regs,pcs.reshape(-1,1)))
...
def gen_dataset(trace_path, dataset_path, hard_brs):
    ...
    pcs, directions, regs = read_branch_trace(trace_path)  #regs:n * 18个uint32

    print('Creating output file', dataset_path)
    fptr = create_new_dataset(dataset_path, pcs, directions, regs)

    for br_pc in hard_brs:
        #find indicies of hard branches
        trace_br_indices = np.argwhere(pcs == br_pc).squeeze(axis=1)
        print('processing branch {} times: {}'.format(hex(br_pc),len(trace_br_indices)))
        fptr.create_dataset(
