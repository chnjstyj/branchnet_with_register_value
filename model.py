...
def flatten_regs_value_torch(x:torch,y,bits:int=32):
  '''
  x_ = torch.empty(x.shape[0],y,dtype=torch.int64,device=x.device)
  l:int = 0
  ...
  num_bits = bits
  x_flat = torch.empty(batch_size, num_features * num_bits, dtype=torch.int64, device=x.device)
  
  for i in range(batch_size):
      x_flat[i] = ((x[i].unsqueeze(1) >> torch.arange(num_bits - 1, -1, -1,device=x.device)) & 1).view(-1)
  
  #x_flat_ = torch.cat((x_flat,torch.empty(batch_size,y-x_flat.shape[1],dtype=torch.int64,device=x.device)),dim=1)
  return x_flat     
...
def extract_slice_history(x, config, global_shift, slice_id):
  ...
  if config['shifting_pooling'][slice_id]:
    ...
    #if config['combined_regs_value']:
    #  return x[:, -slice_size-18:-18]
    #else:
      return x[:, -slice_size:]

class Slice_regs(nn.Module):
  def __init__(self,config):
    super(Slice_regs,self).__init__()  
    if config['combined_regs_value_improved'] == True:
      regs_value_length = config['regs_value_length'][0]
      conv_filters = config['regs_value_conv_filters'][0]
      conv2_filters = config['regs_value_conv_filters'][1]
      conv_width = config['regs_value_conv_widths'][0]
      conv2_width = config['regs_value_conv_widths'][1]
      pooling_width = config['regs_value_pooling_widths'][0]
      pooling2_width = config['regs_value_pooling_widths'][1]
      #embedding_dims = config['embedding_dims']
      self.config = config 
      #self.embedding_table = nn.Embedding(2,embedding_dims)
      self.conv = nn.Conv1d(1,conv_filters,conv_width)
      self.batchnorm = nn.BatchNorm1d(conv_filters)
      self.pooling = nn.AvgPool1d(pooling_width,padding=0)   #(582 - 7 + 1) / 6 = 96
      self.conv2 = nn.Conv1d(conv_filters,conv2_filters,conv2_width)
      self.batchnorm2 = nn.BatchNorm1d(conv2_filters)
      self.pooling2 = nn.AvgPool1d(pooling2_width,padding=0) # 96 / 8 = 12 
      pooling_output_size = (((regs_value_length - conv_width + 1) // pooling_width) - conv2_width + 1) // pooling2_width
      self.total_output_size = pooling_output_size * conv2_filters
    else :
      regs_value_length = config['regs_value_length'][0]
      conv_filters = config['regs_value_conv_filters'][0]
      conv_width = config['regs_value_conv_widths'][0]
      ...
      self.conv = nn.Conv1d(1,conv_filters,conv_width)
      self.batchnorm = nn.BatchNorm1d(conv_filters)
      self.pooling = nn.AvgPool1d(pooling_width,padding=0)   #(582 - 7 + 1) / 48 = 12
      pooling_output_size = ((576 - conv_width + 1) // pooling_width)
      self.total_output_size = pooling_output_size * conv_filters
      
  def forward(self,x):
    #self.pooling_batchnorm = nn.BatchNorm1d(self.config['regs_value_conv_filters'][0] * 2,device=x.device)
    hardsigmoid_act = nn.Hardtanh(min_val=0.0, max_val=1.0)
    pooling_width = self.config['regs_value_pooling_widths'][0]
    #x = self.embedding_table(x)
    #x = torch.transpose(x,1,2)
    if self.config['combined_regs_value_improved'] == True:
      self.pooling_batchnorm = nn.BatchNorm1d(self.config['regs_value_conv_filters'][1],device=x.device)
      x = x.float()
      x = x.unsqueeze(1)
      x = self.conv(x)
      x = self.batchnorm(x)
      ...
      self.pooling_batchnorm = nn.BatchNorm1d(self.config['regs_value_conv_filters'][0],device=x.device)
      x = x.float()
      x = x.unsqueeze(1)
      x = self.conv(x)
      x = self.batchnorm(x)
...
class Slice(nn.Module):
  ...
  def hash_using_metadata(self, x, conv_width):
    ...
    available_history = x.shape[1]  #[44, 92, 182]
...
class BranchNet(nn.Module):
  ...
  def __init__(self, config, training_phase_knobs):
    ...
    if (config['combined_regs_value'] == True):
      self.slice_regs = Slice_regs(config)
      concatenated_slices_output_size += self.slice_regs.get_output_size()
  ...
  def forward(self, x):
    ...
    if (self.config['combined_regs_value'] == True):
      temp = x 
      x = x[:,:-18]
      if (self.config['combined_regs_value_18'] == True):
        y = temp[:,-18:-8]
        y = flatten_regs_value_torch(y,self.config['regs_value_length'][0])
        #y.to(temp.device)
        y = self.slice_regs(y)
      else:
        y = temp[:,-18:]
        if (self.config['combined_regs_value_lowbits'] == True):
          y = flatten_regs_value_torch(y,self.config['regs_value_length'][0],8)
        else:
          y = flatten_regs_value_torch(y,self.config['regs_value_length'][0])
        #y.to(temp.device)
        y = self.slice_regs(y)
