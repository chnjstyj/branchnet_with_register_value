...
class ModelWrapper():
  ...
  def train(self, training_traces, training_steps, learning_rate,
    ...
    for num_steps in training_steps:
      ...
      while step < num_steps:
        for inps, labels, regs in training_set_loader:
          inps_c = torch.cat([inps, regs], dim=1)
          outs = self.model(inps_c.to(self.device))
  ...
  def eval(self, trace_list):
    ...
    if test_set_loader is not None: 
      for inps, labels, regs in test_set_loader:
        inps_c = torch.cat([inps, regs], dim=1)
...
def full_evaluation(model_wrapper, include_training_set=False):
  ...
  with open(output_path, 'a') as f:
    for trace in traces:
      ...
      print('{} accuracy of {}: {} out of {} ({}%)'.format(hex(__args__.br_pc),
